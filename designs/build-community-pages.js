...
<section class="community-hero compact-hero wrap" aria-labelledby="scholarship-title"><p class="eyebrow">CHE / Scholarships</p><h1 id="scholarship-title">Educational <em>scholarship.</em></h1><p class="community-subtitle">For high school and undergraduate students pursuing careers in health.</p></section>
<section class="cycle-panel wrap" aria-labelledby="cycle-title"><div><span class="status-label"><span aria-hidden="true"></span>Not yet announced</span><h2 id="cycle-title">Next application cycle</h2><p>CHE hopes to offer the scholarship again. Dates, award amounts, and eligibility are still being confirmed.</p></div><a class="button" href="mailto:calchescholarships@gmail.com">Scholarship questions <span aria-hidden="true"><i>↗</i></span></a></section>
<section class="recipients-section wrap" aria-labelledby="recipients-title"><div class="community-section-heading"><h2 id="recipients-title">Past recipients</h2><p class="sample-note">Sample profiles below; recipient details pending.</p></div><div class="recipient-grid">${recipientSamples.map((member,i)=>`<article class="recipient-card"><div class="recipient-avatar tone-${i}" aria-hidden="true">${member.name.charAt(0)}</div><div><p class="eyebrow">${member.category}</p><h3>${member.name} <span class="sample-tag">Sample profile</span></h3><p>${escape(member.bio)}</p></div></article>`).join('')}</div></section>
<aside class="scholarship-background wrap" aria-label="Scholarship history"><p>The scholarship was previously known as the Familia Paredes-Miramontes scholarship. The family began fundraising with CHE on campus in 2008.</p><a class="text-link" href="${source}" target="_blank" rel="noopener noreferrer">Scholarship history <span aria-hidden="true">↗</span></a></aside>
`);
...
const cards = board.members.map((member,i) => {
  ...
  return `<article class="board-card tone-${i%3}" aria-labelledby="member-${i}"><div class="board-portrait">${member.photo ? `<img src="${escape(member.photo)}" alt="Portrait of ${escape(member.name)}" width="800" height="960" loading="lazy">` : `<div class="portrait-placeholder" aria-hidden="true"><span class="portrait-ring"></span><span class="portrait-head"></span><span class="portrait-body"></span><span class="portrait-star">✳</span></div>`}${board.sampleProfiles ? '<span class="portrait-label">Sample profile</span>' : ''}</div><div class="board-card-copy"><p class="eyebrow">${escape(member.role)}</p><h2 id="member-${i}">${escape(member.name)}</h2><p class="board-bio">${escape(member.bio)}</p></div></article>`;
}).join('\n');
